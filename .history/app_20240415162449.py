...
try:
    ...
    clothes_collection = cxn[os.getenv("MONGO_COLLECTION")]
...
def read():
    ...
    """
    docs = clothes_collection.find({}).sort(
        "created_at", -1
...
def create_post():
    ...
    Type_of_Clothes = request.form["type"]
    ...
    doc = {"name": name, "Type_of_Clothes": Type_of_Clothes, "Brand": Brand, "Size": Size, "Condition": Condition, "Image_URL": Image_URL, "message": message, "created_at": datetime.datetime.utcnow()}
    clothes_collection.exampleapp.insert_one(doc)  # insert a new document
...
def edit(mongoid):
    ...
    """
    doc = clothes_collection.exampleapp.find_one({"_id": ObjectId(mongoid)})
...
def edit_post(mongoid):
    ...
    dclothes_collection.exampleapp.update_one(
...
def delete(mongoid):
    ...
    """
    clothes_collection.exampleapp.delete_one({"_id": ObjectId(mongoid)})
    return redirect(
        url_for("read")
...
def handle_error(e):
    ...
    """
    return render_template("error.html", error=e, doc={})  # render the edit template
